...
_SQUIRRELDEF(SQLite_Prepare) {
	if (sq->gettop(v) == 3 &&
		sq->gettype(v, 2) == OT_USERPOINTER && sq->gettype(v, 3) == OT_STRING) {
		SQUserPointer data;
		sq->getuserpointer(v, 2, &data);
		const char* query;
...
_SQUIRRELDEF(SQLite_GetColumnData) {
	if (sq->gettop(v) == 3 &&
		sq->gettype(v, 2) == OT_USERPOINTER && sq->gettype(v, 3) == OT_INTEGER) {
		...
		if (columnIndex >= 0) {
			...
			if (columnIndex <= INT_MAX) {
				...
				if (colIdx < sqlite3_column_count(stmt)) {
					...
					case SQLITE_TEXT:
					case SQLITE_BLOB:
						sq->pushstring(v, (const char*)sqlite3_column_text(stmt, colIdx), -1);
						return 1;
					}
				} else {
					return sq->throwerror(v, "column index is out of bounds");
				}
#ifdef _SQ64
			} else {
				return sq->throwerror(v, "column index is too big");
			}
#endif
		} else {
			return sq->throwerror(v, "column index is negative");
		}
	}
...
_SQUIRRELDEF(SQLite_EscapeString) {
	if (sq->gettop(v) == 2 &&
		...
		if (result) {
			...
			return sq->throwerror(v, "sqlite3_mprintf() failed to allocate memory -- not your fault!");
		}
	}
